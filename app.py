...
def login():  # ESta función
    ...
    try:
        if request.method == 'POST':
            ...
            user = db.execute(
                'SELECT * FROM usuarios WHERE nombre_usu=?', (username,)).fetchone()
...
def mod_usuario():
    try:
        if request.method == 'POST':
            ...
            lista_usu = ['Id Usuario a Modificar', 'Nuevo Nombre Usuario',
                         'Nuevo Correo Usuario', 'Nuevo Password Usuario']
...
def usuarios():
    ...
    usuarios = get_db().execute(
        'SELECT id_usu, nombre_usu, correo_usu, rol_usu_fk FROM Usuarios').fetchall()
...
def productos():
    ...
    productos = get_db().execute(
        'SELECT id_pro, nombre_pro, cantidad_pro, costo_pro, descripcion_pro, image_pro FROM Productos').fetchall()
    lista_productos = []
...
def mod_producto():
    try:
        if request.method == 'POST':
            ...
            lista_prod = ['Id Producto a Modificar', 'Nuevo Nombre Producto', 'Nueva Cantidad Producto',
                          'Nueva Costo Producto', 'Nueva Descripción Producto', 'Nueva Imagen Producto']
...
if __name__ == '__main__':
    app.run(host='127.0.0.1', port=443, ssl_context=(
        'micertificado.pem', 'llaveprivada.pem'))
